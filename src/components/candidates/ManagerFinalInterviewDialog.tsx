...
import { CalendarIcon, Send, Save, Loader2, Award } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
import { FirstInterviewEvaluationData } from "./ManagerFirstInterviewDialog";

// 7 evaluation criteria (same as First Interview)
...
interface ManagerFinalInterviewDialogProps {
  open: boolean;
...
export function ManagerFinalInterviewDialog({
  open,
  onOpenChange,
  candidateName,
  position,
  ...
  onSave,
  onSubmitAndSend,
}: ManagerFinalInterviewDialogProps) {
          ...
          <Form {...form}>
            <form className="space-y-6">
              ...
              <div className="p-4 bg-secondary/10 rounded-lg space-y-2">
                ...
                {firstInterviewScore && (
                  <div className="mt-2 pt-2 border-t border-secondary/20">
                    <span className="text-muted-foreground text-sm">คะแนน First Interview:</span>{" "}
                    <Badge className="bg-primary text-white ml-2">{firstInterviewScore}/70</Badge>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-4">
                <div>
                  <h3 className="font-semibold text-sm mb-2">2. การประเมิน Final Interview</h3>
                  <p className="text-xs text-muted-foreground mb-4">
                    • ระดับการประเมินแต่ละหัวข้อ คะแนน 1-10 (น้อยที่สุด = 1, มากที่สุด = 10)<br />
                    • คะแนนประเมินอย่างน้อย 45 คะแนน จากคะแนนเต็ม 70 คะแนน ถือว่าผ่านเกณฑ์รับเข้าทำงาน
                  </p>
                </div>

                {/* Evaluation Table */}
                <div className="border rounded-lg overflow-hidden border-secondary/30">
                  {/* Header */}
                  <div className="grid grid-cols-[1fr_100px] gap-2 bg-secondary/20 p-3 font-semibold text-xs border-b-2 border-secondary/30">
                            ...
                            <FormItem>
                              <FormControl>
                                <Input
                                  ...
                                  max="10"
                                  placeholder="0-10"
                                  className="text-center h-9 border-secondary/30 focus:border-secondary"
                                  {...field}
                  ...
                  {/* Total Score */}
                  <div className="grid grid-cols-[1fr_100px] gap-2 bg-secondary/10 p-3 border-t-2 border-secondary/30">
                    <div className="text-xs font-semibold">
                      รวมคะแนน Final Interview<br />
                      <span className="text-[10px] font-normal">
                        [ เกณฑ์คะแนน ≥ 45 ผ่านเกณฑ์รับเข้าทำงาน, &lt; 45 ไม่ผ่านการสัมภาษณ์ ]
                      </span>
                    </div>
                  ...
                  <div className="grid grid-cols-[1fr_100px] gap-2 p-3 bg-muted/10">
                    <div className="text-xs font-semibold">ผลการประเมิน Final Interview:</div>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Textarea
                          placeholder="ใส่ข้อคิดเห็นเพิ่มเติมจากการสัมภาษณ์ Final Interview"
                          rows={4}
                          {...field}
                        />
                      </FormControl>
